...
ger_id_dict = ["BUNDESREPUBLIK DEUTSCHLAND", "REPUBLIQUE", "FEDERAL", "PERSONALAUSWEIS", "IDENTITY", "CARD", "FEDERAL", "REPUBLIC", "FEDERALE"]
    ...
    for block in blocks:
        if block['BlockType'] == 'LINE':
            ...
            if block['Text'].isupper() or re.match(r"[\d]{2}.[\d]{2}.[\d]{4}", block['Text']) or re.match(r"[0-9]+", block['Text']):
                if not any(s in block['Text'] for s in ger_id_dict):
                    for pt in block['Geometry']['Polygon']:
                        x = pt['X']*width
                        y = pt['Y']*height
                        if not (50 <= x <= 565 and 5 <= y <= 160):
                            if not (50 <= x <= 1060 and 5 <= y <= 100):
                                temp = [int(x),int(y)]
                                poly.append(temp)
                    if len(poly) > 0:
                        # crop
                        ...
                        cv2.imwrite("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\pictures_idcard\\lines\\" + str(index) + ".jpg", dst2)
                        index = index + 1
...
def hide_signature(img):
    height, width, _ = img.shape

    cv2.rectangle(img, (750, 875), (1660, 1080), (255, 255, 255), cv2.FILLED)
    # show("no sig", img)
    return img
...
def extract_variable_lines(img, resp, name):
            ...
            if block['Text'].isupper() or re.match(r"[\d]{2}.[\d]{2}.[\d]{4}", block['Text']) or re.match(r"[0-9]+", block['Text']):
                if not any(block['Text'] in s for s in ger_id_dict):
                    for pt in block['Geometry']['Polygon']:
                        x = pt['X']*width
                        y = pt['Y']*height
                        if not 50 <= x <= 565 and not 5 <= y <= 160:
                            if not 50 <= x <= 1060 and not 5 <= y <= 100:
                                temp = [int(x),int(y)]
                                poly.append(temp)   
                        else:
                            print("In restricted area! No Whitebars needed.") 
    ...
    # show("white bars", img)
    #cv2.imwrite("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\preprocessing\\" +  name + ".png", img)
    return img
...
def crop_face(img, cascPath, name):
    ...
    #     cv2.rectangle(img, (x-60, y-100), (x+w+60, y+h+95), (255, 255, 255), cv2.FILLED)

    ######################### WITH STATIC COORDINATES ########################
    cv2.rectangle(img, (97, 221), (690, 1007), (255, 255, 255), cv2.FILLED)
        

    # show("Faces found", img)
    cv2.imwrite("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\pictures_idcard\\thesis_test\\" + name + "_no_face_square.jpg", img)
...
def check_lines():
    ...
    img = cv2.imread("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\preprocessing\\line8.jpg")
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def check_mrz():
    # Process image
    mrz = read_mrz("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\pictures_idcard\\persorueckseite2.jpg", save_roi=True)
    # Obtain image
...
if __name__ == "__main__":
    # for filename in os.listdir("C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\pictures_idcard\\thesis_test"):
    #     src = "C:\\Users\\tim.reicheneder\\Desktop\\Bachelorthesis\\impl_final\\pictures_idcard\\thesis_test\\" + filename  

    #     # front
    #     img = detect_card(src)
    #     img = hide_signature(img)
    ...
    #     # back
    #     img_back = detect_card("C:\\Users\\tim.reicheneder\\Desktop\Bachelorthesis\\impl_final\\pictures_idcard\\back\\ausweis1.jpg")
